...
export default function ExpenseDetailPage() {
  ...
  const load = () => {
    if (id) api.expense(id).then(setExp).catch(() => setError("Ausgabe nicht gefunden"));
  };
  ...
  const handleDownloadReceipt = async () => {
    ...
    try {
      ...
      const fileName = exp.receiptPath
        ? exp.receiptPath.split("/").pop() ?? "beleg"
        : "beleg";
      ...
      setError("Beleg konnte nicht heruntergeladen werden.");
    } finally {
  ...
  if (error && !exp) return (
    <div className="p-8">
      <div className="bg-red-50 text-danger px-4 py-2 rounded-lg text-sm">{error}</div>
    </div>
  );
    ...
    <div className="p-8 max-w-3xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <span className={`text-xs px-2 py-1 rounded-full ${exp.status === "Booked" ? "bg-green-50 text-success" : "bg-yellow-50 text-warning"}`}>
            {exp.status === "Booked" ? "Gebucht" : "Entwurf"}
          </span>
        </div>
      ...
      {!editing ? (
        <div className="bg-surface rounded-xl border border-border p-6 mb-6">
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            {exp.isRecurring && (
              <>
                <div>
                  <span className="text-muted">Wiederkehrend:</span>
                  <span className="font-medium ml-1 text-blue-700">
                    {{ Monthly: "Monatlich", Quarterly: "Quartalsweise", Yearly: "Jährlich" }[exp.recurringInterval as string] || exp.recurringInterval}
                  </span>
                </div>
                <div>
                  <span className="text-muted">Nächste Ausführung:</span>
                  <span className="font-medium ml-1">{exp.recurringNextDate ? new Date(exp.recurringNextDate).toLocaleDateString("de") : "–"}</span>
                </div>
              </>
            )}
          </div>
        ...
        <div className="bg-surface rounded-xl border border-border p-6 mb-6 space-y-3">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-muted block mb-1">Lieferant</label>
              <input value={form.supplier} onChange={e => setForm({ ...form, supplier: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm bg-background" />
            </div>
            <div>
              <label className="text-xs text-muted block mb-1">USt-ID Lieferant</label>
              <input value={form.supplierTaxId || ""} onChange={e => setForm({ ...form, supplierTaxId: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm bg-background" />
            </div>
            <div>
              <label className="text-xs text-muted block mb-1">Kategorie</label>
              <select value={form.expenseCategoryId} onChange={e => setForm({ ...form, expenseCategoryId: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm bg-background">
                <option value="">Waehlen...</option>
                {cats.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}
              </select>
            </div>
            <div>
              <label className="text-xs text-muted block mb-1">Netto</label>
              <input type="number" step="0.01" value={form.netAmount} onChange={e => setForm({ ...form, netAmount: +e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm bg-background" />
            </div>
            <div>
              <label className="text-xs text-muted block mb-1">MwSt %</label>
              <select value={form.vatPercent} onChange={e => setForm({ ...form, vatPercent: +e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm bg-background">
          ...
          <div>
            ...
            <textarea value={form.description || ""} onChange={e => setForm({ ...form, description: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm bg-background" />
          </div>
          ...
          {form.isRecurring && (
            <div className="grid grid-cols-2 gap-3 bg-blue-50 rounded-lg p-3">
              <div>
                <label className="text-xs text-muted block mb-1">Intervall</label>
              ...
              <div>
                <label className="text-xs text-muted block mb-1">Nächste Ausführung</label>
                <input type="date" value={form.recurringNextDate} onChange={e => setForm({ ...form, recurringNextDate: e.target.value })} className="w-full border border-border rounded-lg px-3 py-2 text-sm bg-background" />
              </div>
            </div>
          )}
          <div className="flex gap-2">
            <button onClick={saveEdit} className="px-4 py-2 bg-primary text-white rounded-lg text-sm">Speichern</button>
...
<div className="bg-surface rounded-xl border border-border p-6">
  ...
  {exp.receiptPath ? (
    <div className="flex items-center gap-3">
      ...
      >
        {downloading ? "Wird heruntergeladen..." : "Beleg herunterladen"}
      </button>
      <button
